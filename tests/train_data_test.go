...
func BenchmarkProcessTrainSample(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		_, _ = service.ProcessTrain(bytesData, "GW")
	}
}
...
func BenchmarkFetchAllTrains(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_, _ = service.FetchAllTrains(token, url, stations, clientRequester)
	}
...
func TestFetchAllTrainsWithMockClient(t *testing.T) {
	...
	mappedData, err := service.FetchAllTrains("token", "url", stations, clientRequester)
	if err != nil {
...
func GenMockClient() (*service.ClientRequester, []byte, error) {
	...
	clientRequester := &service.ClientRequester{
		Client: mockClient,
	}
